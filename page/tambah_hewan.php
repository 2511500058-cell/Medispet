...
<head>
    ...
    <title>Tambah Pasien Baru - Medispet</title>
    <link href="../vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="../assets/css/fontawesome.css">
</head>
<body class="bg-light p-4">
    <div class="container bg-white p-4 shadow-sm rounded" style="max-width: 700px;">
        <h5 class="fw-bold mb-4"><i class="fa fa-plus me-2 text-primary"></i>Pendaftaran Pasien Hewan Baru</h5>
        
        <form method="POST" action="" class="row g-3">
            <div class="col-md-6">
                <label class="form-label small fw-bold text-muted">Nama Peliharaan / Hewan</label>
                <input type="text" name="nama_hewan" class="form-control" placeholder="Contoh: Milo, Kitty" required>
            </div>
            <div class="col-md-6">
                <label class="form-label small fw-bold text-muted">Spesies (Jenis Hewan)</label>
                <input type="text" name="spesies" class="form-control" placeholder="Contoh: Kucing, Anjing, Burung" required>
            </div>
            <div class="col-md-6">
                <label class="form-label small fw-bold text-muted">Ras / Varietas</label>
                <input type="text" name="ras" class="form-control" placeholder="Contoh: Anggora, Persian, Golden" required>
            </div>
            <div class="col-md-6">
                <label class="form-label small fw-bold text-muted">Tanggal Lahir (Estimasi)</label>
            ...
            <div class="col-md-12">
                ...
                <select name="id_pemilik" class="form-select" required>
                    ...
                    while($p_row = mysqli_fetch_assoc($p_res)) {
                        echo "<option value='{$p_row['ID_Pemilik']}'>{$p_row['Nama_Pemilik']} (ID: PM-{$p_row['ID_Pemilik']})</option>";
                    }
                    ?>
                </select>
                <small class="text-danger">*Pastikan pemilik sudah didaftarkan terlebih dahulu di halaman Kelola Pemilik.</small>
            </div>
