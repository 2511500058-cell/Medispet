...
// Ambil ID dari URL (Contoh: edit_hewan.php?id=1)
$id_hewan = isset($_GET['id']) ? $_GET['id'] : 0;

// Tarik data hewan yang sudah ada
...
if (isset($_POST['update'])) {
    ...
    if (mysqli_query($koneksi, $query_update)) {
        echo "<script>alert('Data hewan berhasil diperbarui!'); window.location.href='data_hewan.php';</script>";
        exit();
    } else {
...
<head>
    ...
    <title>Edit Pasien Hewan - Medispet</title>
    <link href="../vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="../assets/css/fontawesome.css">
</head>
<body class="bg-light p-4">
    <div class="container bg-white p-4 shadow-sm rounded" style="max-width: 800px;">
        <h5 class="fw-bold mb-4"><i class="fa fa-edit me-2 text-warning"></i>Edit Profil Pasien Hewan</h5>
        ...
        <form method="POST" action="" class="row g-3">
            ...
            <div class="col-md-12 mt-4">
                ...
                <select name="id_pemilik" class="form-select" required>
                    ...
                    while($p_row = mysqli_fetch_assoc($p_res)) {
                        // Menandai pemilik sebelumnya agar tidak berubah secara acak
                        $selected = ($p_row['ID_Pemilik'] == $data['ID_Pemilik']) ? 'selected' : '';
                        echo "<option value='{$p_row['ID_Pemilik']}' $selected>{$p_row['Nama_Pemilik']} (ID: {$p_row['ID_Pemilik']})</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <div class="col-12 mt-5 d-flex justify-content-between">
                ...
                <button type="submit" name="update" class="btn btn-warning px-4 fw-bold">Update Profil</button>
            </div>
        </form>
    </div>
</body>
